...
public class MockHttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
	private static final Logger LOG = LoggerFactory.getLogger(MockHttpServerHandler.class);
	...
	//following fields only used for registered response
	private final boolean isRegistered;
	private HttpMethod regMethod;
	private String regPath;
	private HttpResponseStatus regResponseStatus;
	private String regRespBody;

	public MockHttpServerHandler() {
		isRegistered = false;
	}

	public MockHttpServerHandler(HttpMethod regMethod, String regPath, HttpResponseStatus regResponseStatus, String regRespBody) {
		isRegistered = true;
		this.regMethod = regMethod;
		this.regPath = regPath;
		this.regResponseStatus = regResponseStatus;
		this.regRespBody = regRespBody;

		if (LOG.isDebugEnabled()) {
			LOG.debug("Register response to method: {}, URI: {}, response status: {}, response body: {}",
					regMethod, regPath, regResponseStatus, regResponseStatus);
		}
	}
	...
	protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
		...
		if (isRegistered && regMethod.equals(method) && StringUtils.equals(regPath, uri)) {
			sendRegisteredResponse(ctx, request);
			return;
	...
	private void sendRegisteredResponse(ChannelHandlerContext ctx, FullHttpRequest request) {
		sendResp(ctx, request, regRespBody, regResponseStatus);
	}
	...
	private void sendResp(ChannelHandlerContext ctx, FullHttpRequest request, String body) {
		sendResp(ctx, request, body, HttpResponseStatus.OK);
	}
	...
	private void sendResp(ChannelHandlerContext ctx, final FullHttpRequest request, Object body, HttpResponseStatus status) {
		sendResp(ctx, request, LOCAL_GSON.get().toJson(body), status);
	}
